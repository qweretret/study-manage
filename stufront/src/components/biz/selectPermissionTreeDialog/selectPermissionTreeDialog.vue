<template>
	...
	<div style="display: flex;">
		...
		<permission-tree :filterable="filterable" ref="permissionTree" :multiple="multiple" :relationType="relationType"
			@confirm="confirm"></permission-tree>
	</div>
</template>

<script>
	import permissionTree from '@/components/biz/selectPermissionTreeDialog/permissionTreeDialog.vue';
	import api from '@/api/components/biz/selectPermissionTreeDialog/selectPermissionTreeDialog.js';
	export default {
		name: 'selectPermissionTreeDialog',
		components: {
			permissionTree,
		},
		data() {
				...
				handler: function(val) {
					if (this.multiple) {
						if (!_.isEmpty(val)) {
							if (this.stringMode) {
								if (_.isArray(val)) {
									console.error("selectPermissionTreeDialog  数据为数组，请使用数组模式。");
									return;
								}
								val = val.split(",");
							} else {
								if (_.isString(val)) {
									console.error("selectPermissionTreeDialog  数据为字符串，请使用字符串模式。");
									return;
								}
							}
						}
		...
		methods: {
			...
			open() {
				this.$refs.permissionTree.open(this.selectedListData, '选择权限', '30%');
			}
		},
		created() {
			this.init();
