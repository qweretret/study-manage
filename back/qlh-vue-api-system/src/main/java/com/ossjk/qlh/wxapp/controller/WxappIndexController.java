...
public class WxappIndexController extends BaseController {
    ...
    @ApiOperation(value = "小程序登录,获取头像")
    @GetMapping("/gethead")
    public ResponseBean gethead(HttpServletRequest request) {
        String token = this.getToekn(request);
        LoginUser uer = systemCommonApi.getCacheLoginUser(token);
        String gethead = iTuisongzuoyeService.gethead(uer.getId());
        return   ResponseBean.Success(gethead);
    }
}
