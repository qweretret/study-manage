<template>
	<popup ref="popup" :loading="popupLoading">
					...
					<el-row>
						<el-col :span="24">
							<el-form-item label="内容" prop="content">
								<el-input type="formParameter.content"
										  :rows="2"
										  placeholder="请输入内容"
		...
		<div slot="footer">
			...
			<el-button v-if="!disabled" type="primary" :loading="confirmLoading" @click="confirm2()">确 定 并 推 送</el-button>
				...
				this.$utils.checkForm(this.$refs.form, () => {
					...
					if (!this.parameter.id) {
						...
						for (var i = 0; i < this.listStu2.length; i++) {
							this.formParameter.sid = this.listStu2[i].id; this.formParameter.whethertopush = 0;
                            this.formParameter.tid = this.listUser.id;
                        ...
                        let parameterss = [];
                        console.log(this.listStu2);
                            this.formParameter.sid = this.listStu2[0].id;
                            let parametera = this.$utils.merger(this.formParameter);
                        api.update(parametera,
							response => {
								this.$utils.msg.success(response.msg);
								this.confirmLoading = false;
								this.close();
        ...
        confirm2() {
            this.$utils.checkForm(this.$refs.form, () => {
                ...
                if (!this.parameter.id) {
                    ...
                    console.log(this.listStu2);
                    for (var i = 0; i < this.listStu2.length; i++) {
                        this.formParameter.sid = this.listStu2[i].id;
                        this.formParameter.tid = this.listUser.id;
                        let parameter = this.$utils.merger(this.formParameter);
                        parameters.push(parameter);
                    }
                    api.save2(parameters,
                        response => {
                            this.$utils.msg.success(response.msg);
                            this.confirmLoading = false;
                            this.close();
