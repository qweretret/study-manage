...
public class SwaggerConfiguration {
    ...
    @Bean(value = "assemblyApi")
    public Docket assemblymApi() {
        Docket docket = new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo())
                // 分组名称
                .groupName("组件模块").select()
                // 这里指定Controller扫描包路径
                .apis(RequestHandlerSelectors.basePackage("com.ossjk.qlh.components")).paths(PathSelectors.any()).build()
                .extensions(openApiExtensionResolver.buildSettingExtensions())
                .securityContexts(CollUtil.newArrayList(securityContext()))
                .securitySchemes(CollUtil.newArrayList(apiKey()));
        return docket;
    ...
    public Docket wxappApi() {
        Docket docket = new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo())
                ...
                // 这里指定Controller扫描包路径
                .apis(RequestHandlerSelectors.basePackage("com.ossjk.qlh.wxapp.controller")).paths(PathSelectors.any()).build()
                .extensions(openApiExtensionResolver.buildSettingExtensions())
                .securityContexts(CollUtil.newArrayList(securityContext()))
                .securitySchemes(CollUtil.newArrayList(apiKey()));
        return docket;
